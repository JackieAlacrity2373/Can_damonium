                                                    ...
                                                    int startSample, int numSamples)
{
    ...
    for (int i = 0; i < numSamples; ++i)
    {
        ...
        // Apply fade envelope (50ms in/out)
        float fade = 1.0f;
        const int fadeLength = static_cast<int> (0.05 * sampleRate);

        if (samplesGenerated < fadeLength)
            fade = samplesGenerated / static_cast<float> (fadeLength);
        else if (samplesGenerated > totalSamples - fadeLength)
            fade = (totalSamples - samplesGenerated) / static_cast<float> (fadeLength);
